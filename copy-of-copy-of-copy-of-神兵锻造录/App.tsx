...
import { Player, Material, Quality, Equipment, EquipmentType, QualityColors, QualityNames, DungeonState } from './types';
...
const App: React.FC = () => {
  ...
  const totalStats = useMemo(() => {
    ...
    const addStat = (item: Equipment | null) => {
      if (!item) return;
      item.stats.forEach(s => {
        (stats as any)[s.type] += s.value;
      });
    };
    addStat(player.equippedWeapon);
    addStat(player.equippedArmor);
    return stats;
  }, [player]);
